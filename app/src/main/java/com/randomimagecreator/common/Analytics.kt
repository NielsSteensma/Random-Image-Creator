...
object Analytics {
    ...
    /**
     * Logs an image creation attempt with given set of [configuration].
     */
    fun imageCreationEvent(configuration: Configuration) {
        analytics?.logEvent(IMAGE_CREATION_EVENT, Bundle().apply {
            putInt(IMAGE_CREATION_EVENT_PARAM_AMOUNT, configuration.amount)
            putInt(IMAGE_CREATION_EVENT_PARAM_WIDTH, configuration.width)
            putInt(IMAGE_CREATION_EVENT_PARAM_HEIGHT, configuration.height)
            putString(IMAGE_CREATION_EVENT_PARAM_FORMAT, configuration.format.name)
        })
    }
}
